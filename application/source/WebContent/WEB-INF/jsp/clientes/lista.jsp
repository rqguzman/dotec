            ...
            <table class="adminlist">
              <thead>
                <tr>
                  ...
                  <th><a href="javascript:void(0)" title="Título">Nome</a> </th>                  
                  <th width="10%"><a href="javascript:void(0);" title="CPF">CPF</a> </th>
                  <th width="5%"><a href="javascript:void(0);" title="CNPJ">CNPJ</a> </th>                  				  <th width="10%"><a href="javascript:void(0);" title="Guarda Técnica">Guarda Técnica</a> </th>	
					...
					<c:otherwise>
						<c:forEach items="${clienteList}" var="cliente" varStatus="rowCounter">
							<tr class="
								...
								<td>									<c:choose>										<c:when test="${cliente.habilitado}">Sim</c:when>										<c:otherwise>Não 																				<c:if test="${userInfo.user.tipoDeUsuario=='ADMINISTRADOR'}">										<a href="#" onclick="enableEmployers(${cliente.id})" id="enableEmployers" title="clique aqui para habiliatdo o usuário"> <img alt="clique aqui para habiliatdo o cliente" src="<c:url value="/client/css/media/system/images/checked_out.png" />" /></a>																				</c:if>										</c:otherwise>									</c:choose>																									</td>							<td>${cliente.diaDeVencimento}</td>
								<td>${cliente.id}</td>
							</tr>
						</c:forEach>
					</c:otherwise>
